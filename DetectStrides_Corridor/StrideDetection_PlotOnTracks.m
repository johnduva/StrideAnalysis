function StrideDetection_PlotOnTracks(final_tracks_c, StrideData, Fs, numLimbs)
    ...
    for paw_i = 1:numLimbs
        ...
        for swing_i = 1:total_strides
            idx = StrideData.swing{paw_i}(swing_i) : StrideData.stance{paw_i}(swing_i);
            plot(idx/Fs, squeeze(final_tracks_c(1,paw_i,idx)),'w','LineWidth',2) % 'w' = white
        end
        
        % distinguish the stances in black
        for stance_i = 1:total_strides-1 
            idx = StrideData.stance{paw_i}(stance_i) : StrideData.swing{paw_i}(stance_i+1);
